...
impl SimpleRenderable for FiboTenth {
    fn render<W: core::fmt::Write>(&mut self, writer: &mut W) {
        write!(writer, "The tenth Fibonacci number is {}", fibonacci(10)).unwrap()
    }
}
...
fn build_handler() -> impl Handler + Reporting {
    new_dispatcher()
        .at(&["fibo_ten"], SimpleRendered(FiboTenth))
        .at(&["fibo_twenty"], SimpleRendered(FiboTwenty))
}
